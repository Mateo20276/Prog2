...
Const
    ...
    MAX = 20; // Tamaño maximo
    Nulo= 0; // Indica posicion invalida de la lista
Type
  PosicionLista = LongInt; // Abstrae la posición de un ítem de la lista.
  Lista = Object
    Private // Atributo
      ...
      Procedure Intercambio (P,Q: PosicionLista); // intercambia el tipoelemento de 2 posiciones  // intercambia el tipoelemento de 2 posiciones de la lista
...
Begin
  Agregar := CError;  // Controla que el Tipo de Dato de la Clave sea Homogeneo a la Lista
...
Var Q: PosicionLista;
Begin
  Insertar := CError; // Controla que el Tipo de Dato de la Clave sea Homogeneo a la Lista
  if X.TipoDatoClave(X.Clave) <> TDatoDeLaClave then Begin
    Insertar := ClaveIncompatible;
    Exit;
  End;
  If EsLLena Then // ahora lo Inserto
    Insertar := LLena
  Else Begin
    ...
    Begin
      ...
      Elementos[P] := X;// Ahora pego el dato en la posicion recibida (P)
      Inc(Final);
      Inc(Q_Items);
      Insertar := OK;
    End
...
Begin
  ...
  Else Begin
    ...
    Begin
      ...
      Dec(Final); // Actualizo el final y la cantidad
      Dec(Q_Items);
