...
Type
  ...
  Public
    ...
    Function eliminarelemento(X: Tipoelemento): Resultado;
    Function eliminarelementologico(P: Integer): Resultado;
...
Begin
  ...
  Begin
    ...
    If (P = Inicio) And (P = Final) Then
      Crear(TDatoDeLaClave, Size) // Unico de la lista. Se crea vacia
    Else Begin
      If (P = Inicio) Then
      Begin                       // Se elimina el primer elemento. Cambia el inicio
        Inicio := Cursor[Inicio].Prox;
        cursor[Inicio].Ante := NULO;
      End
      Else
        If (P = Final) Then
        Begin   // Se elimina el ultimo elemento. Cambia el Final
...
Begin
  ...
  Begin
    If Cursor[Q].Datos.Clave <> X.Clave Then
      Q := Cursor[Q].Prox
    Else
      Encontre := True;
  End;
...
Begin
  ...
  If (P <> NULO) AND ValidarPosicion(P) Then
  Begin
  ...
  Begin
    ...
    begin
      ...
      Begin
        ...
        if Ascendente then if X1.Clave > X2.Clave then
          Intercambio(Q, Cursor[Q].Prox);
        if Not Ascendente then if X1.Clave < X2.Clave then
          Intercambio(Q, Cursor[Q].Prox);
      End;
    Q := Cursor[Q].Prox;
    End;
...
Function Lista.eliminarelemento (X:Tipoelemento): Resultado;
Var Q,C: PosicionLista;
    E: TipoElemento;
begin
Q:= Inicio;
E:= Cursor[Q].Datos;
eliminarelemento:= Cerror;
  while Q <> Nulo do
  begin
    C:= cursor[Q].prox;
    if X.clave = E.clave then
    begin
      eliminar(Q);
    end;
...
Function Lista.eliminarelementologico (P: Integer): Resultado;
Var Q: Integer;
begin
Q:= 1;
eliminarelementologico:= Cerror;

  while Q <> size do
  begin
    if Q = P then
    begin
      Cursor[Cursor[Q].Ante].Prox := Cursor[Q].Prox;
      Cursor[Cursor[Q].Prox].Ante := Cursor[Q].Ante;
      Cursor[Q].Prox := Libre;
      Libre := Q;
    end;
    Q:= Q + 1;
